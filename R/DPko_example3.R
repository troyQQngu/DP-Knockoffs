# In this experiment we will use a smaller threashold w so the regulation will
# be milder and hopefully we can get a better result
...
experiment3 <- function(n) {
  ...
  for (i in 1:num_iter){
    ...
    UDV <- svd(A)
    min_sin <- min(UDV$d)
    s <- min_sin^2+Z-2*B^2*log(2/delta)/epsilon
