% DP Knockoffs Experiment 6

list_len = 21;

% parameters
mu_list = linspace(0,0.25,list_len); % magnitude of signals
p_list = 40; % parameter size
n_list = 1000000; % sample size
r_list = 1500; % rotation dimension
c0 = 0.5; % sparsity coefficient
...
T = 0.0001; % selection threshold
lambda = 0.03; % l1 penalty
n_iter = 1;% number of iteration for knockoff power and fdr estimation
...
% Experiment
for i = 1:list_len
    [mean_power,fdr,power_th,fdr_th,fdr_est_th] = DPknockoffs_ker(n_list,p_list,s0_list,mu_list(i),r_list,eps,del,b,n_iter,T,lambda,gamma,c0);
