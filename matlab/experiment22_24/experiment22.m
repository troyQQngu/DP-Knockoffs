% experiment 22. In this experiment, we want to compare the JLT method to
% AG method. For the AG method, we record 3 kinds of output. a)No selection
% if objective function is not PSD, average over all trials. b)Use coordinate
% descent even if objective funciton is not PSD, average over all trials.
% c) Use coordinate descent even if objective function is not PSD, average
% over convergent trails.

list_len = 18;

% parameters
mu_list = 0.5; % magnitude of signals
p_list = 50; % parameter size
n_list = floor(linspace(30000,200000,list_len)); % sample size
